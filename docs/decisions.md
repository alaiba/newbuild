...
| Phase-1 memory capacity | **64 GB, 2×32 GB** | **Selected** | Final BOM review leaves substantial room below the ~30k planning level. The additional spend over 32 GB buys both double capacity and dual-channel bandwidth without displacing any permanent component, which is worthwhile for heavy Java/Android/IDE/container/VM use. |
| Phase-1 memory target | **Crucial `CT2K32G56C46U5`, 64 GB (2×32), DDR5-5600 CL46, 1.1 V** | **Selected target** | Conservative JEDEC-class electrical profile, dual-channel topology and low-profile bare modules remain the preferred balance. Current Romanian pricing is volatile around the ~5.0k class. Kingston `KF556C36BBEK2-64` is the explicit-ProArt compatibility fallback; recompare it if the Crucial price approaches Kingston. Strict purchase gate: exact desktop `...U5` SKU, never the laptop/SO-DIMM `...S5` variant. Boot at Auto/JEDEC and validate extensively. |
...
| Host operating system | **Windows 11 Pro x64** | **Selected** | Best fit for native IntelliJ/Android Studio, NVIDIA/gaming compatibility, BitLocker, Remote Desktop hosting and Hyper-V/WSL2. Home drops professional host features; Pro for Workstations adds no material value for this one-socket/256-GB-target build. |
| Windows license procurement | **Microsoft Windows 11 Pro Retail/FPP USB English `HAV-00163`** | **Selected / required purchase** | No existing transferable Windows license is available. For a strict Microsoft-compliant DIY self-build, use Retail/FPP rather than personally purchasing an OEM/System Builder license. Current normal Romanian retailer controls are roughly 1.12–1.15k lei, below Microsoft's 1,199 RON direct reference. Do not substitute OEM/DSP or an undocumented standalone key. |
