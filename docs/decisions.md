...
| Memory architecture target | Preserve a credible path to **256 GB**, while allowing the initial purchase to start lower | **Selected** | The platform, motherboard and memory validation work should preserve a stable 256 GB endpoint. 256 GB is not required to be purchased on day one if current pricing is disproportionate. Do not weaken the motherboard/platform merely to reduce initial memory cost. |
| Phase-1 memory floor | **32 GB minimum; commissioning memory may be any stable 32 GB-or-larger configuration** | **Selected** | The current workload is already being handled on 32 GB, so Phase 1 does not need to buy additional capacity merely to preserve the rest of the build. Treat Phase-1 RAM as temporary/replaceable. If buying new, 2×16 GB is preferred when similarly priced because it preserves dual-channel bandwidth, but 1×32 GB or another simple 32 GB+ configuration is acceptable if it is the lowest-cost stable way to commission the system. Do not let the temporary kit constrain the eventual 256 GB configuration. |
...
- exact Phase-1 memory kit/configuration at 32 GB or more
