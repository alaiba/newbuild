...
| Motherboard | **Exact motherboard reopened; ASUS ProArt X870E-Creator WiFi remains the incumbent reference** | **Reopened** | The ProArt was promoted largely because of the difficult 4×64 GB / 256 GB requirement. That requirement is now removed. Re-optimize the board against the final 128 GB 2×64 GB / 1DPC target and the remaining workstation requirements before purchase. No replacement is selected yet. |
...
| Storage architecture | **2 TB system/tools SSD now + future 4 TB+ work SSD** | **Selected, pending requested storage review** | Current architecture gives OS/recovery separation and preserves a future high-capacity work drive. The user explicitly requested a storage-architecture optimization discussion before the next whole-build optimization pass; no storage change is selected yet. |
| System SSD | **Samsung 990 PRO 2 TB `MZ-V9P2T0BW` in `M.2_3`** | **Selected, pending requested storage review** | Mature Gen4 TLC+DRAM system-drive choice. Keep as incumbent while evaluating whether a different capacity/role/topology is better once the storage architecture is reconsidered. |
...
| Cost philosophy | **Optimize utility per leu, including justified premium upgrades** | **Selected** | Utility includes stability, endurance, serviceability, warranty, thermal/electrical margin, workload performance and avoiding future replacement. |
...
## Explicitly superseded memory decisions
...
- exact motherboard after re-optimization for the 128 GB 1DPC endpoint;
- exact matched 2×64 GB RAM kit and ECC/non-ECC verdict;
- storage architecture review requested before the next whole-build optimization pass;
